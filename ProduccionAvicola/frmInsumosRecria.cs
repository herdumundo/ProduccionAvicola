...
{
    public partial class frmInsumosRecria : Form
    {
        ...
        private void btnAceptar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                sql_lotes = sql_lotes + " from oitw a with (nolock) ";                
                sql_lotes = sql_lotes + " inner join OITM b with (nolock) on a.ItemCode = b.ItemCode ";
        ...
        private void txtArticulo_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.F2)
            {
                ...
                frm.sql = frm.sql + " and (b.ItemCode like 'inre%' or b.ItemCode like 'LENA%' or b.ItemCode like 'LIMP%') ";
                ...
                frm.sql = frm.sql + " from oitw a with (nolock) ";
                frm.sql = frm.sql + " inner join oitm b with (nolock) on a.itemcode = b.itemcode ";
                frm.sql = frm.sql + " inner join obin c with (nolock) on a.WhsCode = c.WhsCode ";
